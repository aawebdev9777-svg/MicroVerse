...
export const SnakeGame: React.FC = () => {
    ...
    useEffect(() => {
        const loadHighScore = async () => {
            if (auth.currentUser) {
                try {
                    const docRef = doc(db, 'users', auth.currentUser.uid, 'games', 'snake');
    ...
    const saveHighScore = async (newScore: number) => {
        if (newScore > highScore && auth.currentUser) {
            setHighScore(newScore);
            try {
                await setDoc(doc(db, 'users', auth.currentUser.uid, 'games', 'snake'), {
                    highScore: newScore,
                    lastPlayed: new Date()
                });
            } catch (e) {
    ...
    useEffect(() => {
        const handleAICommand = (e: CustomEvent) => {
            ...
            // Trigger action for a short duration
            if (action === 'move_left') {
                state.current.keys.left = true;
                setTimeout(() => { state.current.keys.left = false; }, 200);
            } else if (action === 'move_right') {
                state.current.keys.right = true;
                setTimeout(() => { state.current.keys.right = false; }, 200);
            } else if (action === 'fire') {
                state.current.keys.space = true;
                setTimeout(() => { state.current.keys.space = false; }, 100);
            }
        };

        window.addEventListener('game-control' as any, handleAICommand);
        return () => window.removeEventListener('game-control' as any, handleAICommand);
    }, [isPlaying]);
    ...
    return (
        <div ref={containerRef} className="h-full w-full bg-zinc-950 relative overflow-hidden flex flex-col items-center justify-center select-none" tabIndex={0}>
            ...
            {highScore > 0 && (
                <div className="absolute top-4 right-4 z-10 font-mono text-yellow-500 font-bold text-sm select-none pointer-events-none drop-shadow-md flex items-center gap-2">
